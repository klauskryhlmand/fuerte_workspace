...
void laserScanCallback(const sensor_msgs::LaserScan::ConstPtr& laser_scan)
{
	...
	for(int i = 0; i < laser_scan->ranges.size(); i++)
	{
		...
		if((laser_scan->ranges[i] <= 0.5 || i == laser_scan->ranges.size()-1) && hole*270.0/laser_scan->ranges.size() > 35.0)
		{
			...
			if(hole*270.0/laser_scan->ranges.size() > 92.0)
			{
...
int main(int argc, char** argv){
	...
	laser_pub = n.advertise<frobo::laser_info>(laser_info_topic.c_str(), 1);
