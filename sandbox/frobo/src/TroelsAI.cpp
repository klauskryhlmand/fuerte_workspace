...
void laserInfoCallback(const frobo::laser_info ls_inf)
{
	FroboMsgs::pwm_o msg;
	msg.direction = 5;
	msg.enable = 0;
...
//***********************************************************************************************/
				...
				if(ls_inf.force > 0)
				{
					...
					msg.speed_left = ceil(Max_speed - ls_inf.force*Max_speed);
				}
				else
				{
					//obstacles to the right;
					msg.speed_right = ceil(Max_speed + ls_inf.force*Max_speed);
				}
...
***********************************************************************************************/
			...
			{
				//obstacles to the right;
				msg.speed_right += floor(ls_inf.force*Max_speed*2);
			}
...
int main(int argc, char** argv){
	...
	laser_sub = n.subscribe<frobo::laser_info>(laser_info_topic.c_str(), 1, &laserInfoCallback);
